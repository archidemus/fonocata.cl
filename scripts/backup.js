...
async function backup() {
    ...
    const tempBackupFolder = path.join(config.backupDir, `backup_temp_${dateStr}`);
    const finalZipPath = path.join(config.backupDir, `backup_raiz_${dateStr}.zip`);
    ...
    try {
        ...
        console.log(`⏳ Esto puede tomar tiempo dependiendo del tamaño del sitio y tu conexión.`);
        ...
        if (fs.existsSync(tempBackupFolder)) {
            try { fs.rmSync(tempBackupFolder, { recursive: true, force: true }); } catch (e) {}
        }
    }
    finally {
        client.close();
